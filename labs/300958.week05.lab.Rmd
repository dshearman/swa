...
In the UWS computer labs (using the Windows operating system), we must tell R not to bother verifying the Twitter SSL certificate:
```{.r}
options(RCurlOptions = list(verbose = FALSE, capath = system.file("CurlSSL", "cacert.pem", package = "RCurl"), ssl.verifypeer = FALSE))
```
If you are not using Windows, you can ignore the above command.
