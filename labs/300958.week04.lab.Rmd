...
column). To perform the test, we need to compute
...
Once we have the data in two column, we shuffle the columns and recompute the table and compute the $\chi^2$ value (as shown in the lecture).
...
of size `n`. Therefore, we can comute the randomisation distribution by:

- sample from the set of age categories with replacement using the given proportions and sample size (using `sample`).
