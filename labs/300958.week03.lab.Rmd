...
There is also a source button above the text editor window.
...
variable (e.g. int x;), the type of the variable is set once data is
assigned. To create a variable `x` of type numeric containing the
value `4`, we issue the command:
```{r}
x = 4
```
...
A data frame is a rectangular table of data. with named5B5B5B columns.  The
