...
the owner's name and id.  Kevin Rudd is a popular person, so he must
...
Kevin Rudd's details, the function returns a list of his friends.  The
...
Once we have the vector `protected.status`, we can extract the unprotected friends of Kevin Rudd:
```{r eval=FALSE}
...
more.friends = list()
```
We can download a list of 100 friends of the first friend of Kevin Rudd using:
```{r eval=FALSE}
more.friends[[1]] = top.unprotected.friends[[1]]$getFriends(100)
```

**Write** a `for` loop to download 100 friends from the 10 most
popular friends of Kevin Rudd and store them in `more.friends`.


## Creating the Twitter Graph

Now we have Kevin Rudd, 100 of Kevin Rudd's friends, and 100 of 10 of
Kevin Rudd's most popular friends.

We can create a graph by constructing an edge list (who links to who).
We know Kevin Rudd links to all of his friends 100, so the edge list
will contain 100 rows beginning with Kevin Rudd and ending with the
friend of Kevin Rudd. First we must get the names of all of Kevin
Rudd's friends. 
...
```{r eval=FALSE}
rudd = rep(user$getScreenName(), length(friends)) # repeat Kevin Rudd's user name 100 times
el = cbind(rudd,friend.names) # bind the columns to create a matrix
```

We need to do the same for Kevin Rudd's 10 most popular friends. To
...
We can use the created function `user.to.edgelist` to create the edge list for Kevin Rudd:
```{r eval=FALSE}
el.rudd = user.to.edgelist(user, friends)
```
...
for (a in c(1:length(more.friends))) {
    ...
    el.rudd = rbind(el.rudd, el.friend) # append the new edge list to the old one.
}
```
Now that we have the edge list, we can create the graph:
```{r eval=FALSE}
g = graph.edgelist(el.rudd)
```
