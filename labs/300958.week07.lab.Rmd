...
5. Then create the term document matrix for the set of tweets.
...
2. Extract the names of the words using the function `names` on the weight vector.
...
stoplist containg the words to be removed and the words from
`stopwords()`.
...
Using the term-document matrix from above, produce visualisation plot of the first two prinicpal components. Remember that we are using a TF-IDF weighted term frequencies.

First we generate a vector of colors to represent the three polical leaders.
...
Examine the PCA summary to identify the variance that the first few componenets represent.
...
The file [SimpleWordle.R](SimpleWordle.R) contains the code for a
simplified version of the word cloud drawing code.  As a challenge,
try modifying this code to

1. Add colour.
2. Add $90^\circ$ rotation of a proportion of words at random.

(Hint: `runif` can generate a uniform random number so that `runif(1)<
p` will be `TRUE` `p` proportion of the time.  Also `text` takes an
argument `srt=90` to write text at 90 degrees).
