...
questions = list(
    list(
      type =  'NUM',
      known.parameters =  'file_name g `r png(paste(image_path, file_name, sep="/")); par(mar = c(0,0,0,0)); V(g)$label.cex = 1.8; plot(g, layout=layout.fruchterman.reingold, vertex.size = 15); dev.off()`',
      question = '<p>Using the following graph, if we start at vertex 1 and take a random walk of length 10,000, what is the probability of us finishing the walk at vertex 1 (given at two decimal places)?</p><img src="`r paste(image_url, file_name, sep="/")`">',
      answer = '`r degree(g)[1]/sum(degree(g))`',
      tolerance = "0.01"
    ),
    list(
      type =  'NUM',
      known.parameters =  'file_name g `r png(paste(image_path, file_name, sep="/")); par(mar = c(0,0,0,0)); V(g)$label.cex = 1.8; plot(g, layout=layout.fruchterman.reingold, vertex.size = 15); dev.off();  A = t(get.adjacency(g)); n = length(V(g)); D = diag(1/apply(A,2,sum)); T = A %*% D; E = matrix(1/n,n,n); P = alpha*T + (1-alpha)*E;`',
      question = '<p>Using the Random Surfer probability transition matrix of the following graph, with &alpha; = `r alpha`, what is the probability of beginning at vertex 1 and moving to vertex 2 in a random walk of length 1 (given at two decimal places)?</p><img src="`r paste(image_url, file_name, sep="/")`">',
      answer = '`r P[2,1]`',
      tolerance = "0.01"
    ),
    list(
      type =  'MC',
      known.parameters =  'file_name g choices correct `r png(paste(image_path, file_name, sep="/")); par(mar = c(0,0,0,0)); V(g)$label.cex = 1.8; plot(g, layout=layout.fruchterman.reingold, vertex.size = 15); dev.off(); `',
      question = '<p>Which of the following edges do we need to add to make the following graph ergodic?</p><img src="`r paste(image_url, file_name, sep="/")`">',
      choices = list(
        '`r choices[1]`',
        '`r choices[2]`',
        '`r choices[3]`',
    ...
    list(
      ...
      question = '<p>Given the following probability transition matrix</p> `r gsub("\\n","",c(print(xtable(T, digits=2), type="html", comment=FALSE, include.colnames=FALSE)))` <p>the stationary distribution is:</p>',
      choices = list(
        '`r choices[1]`',
        '`r choices[2]`',
        '`r choices[3]`',
...
yaml.out(questions[[3]], 'file_name = "fahD2ogh1ru2eidook1I.png"; g = graph.formula(1+-2, 1+-3, 1+-+4, 4-+3); choices = c("2->3","3->2","1->3","2->4"); correct = 2')
...
yaml.out(questions[[4]], 'T = matrix(c(0,0.5,0.5,0.5,0.4,0.1,1,0,0),3,3); pv = function(s) {paste("[", paste(s, collapse=" "), "]")}; e = eigen(T); s = round(e$vectors[,1]/sum(e$vectors[,1]), digits = 2); choices = c(pv(s[c(2,1,3)]), pv(s[c(1,2,3)]), pv(s[c(3,2,1)]), pv(s[c(3,1,2)])); correct = 2')
