...
0$, we have:
$$
	P(X = x\vert C = 1) \text{ and } P(X = x\vert C = 0)
$$
If we obtain a new sample $x$, we want to compute:
$$
	P(C = 1\vert X = x) \text{ and } P(C = 0\vert X = x)
$$
...
but we have:
$$
	P(X\vert C = 1) \text{ and } P(X\vert C = 0)
$$
Therefore, we use Bayes' rule:
$$
	P(C = 1\vert X = x)  = \frac{P(X\vert C = 1)P(C = 1)}{P(X = x)}
$$

- $P(C = 1\vert X = x)$: Probability class 1, given the sample $x$.
- $P(X = x\vert C = 1)$: Probability of sampling $x$, given the class of the sample is 1.
...
$$
	L = \log{\left (\frac{P(C = 1\vert X = x)}{P(C = 0\vert X = x)}\right )} = 
	\log{\left (\frac{P(X = x\vert C = 1)P(C = 1)}{P(X = x\vert C = 0)P(C = 0)}\right )}
$$
which can be separated into:
$$
	L = \log{\left (\frac{P(C = 1)}{P(C = 0)}\right )} + 
	\log{\left (\frac{P(X = x\vert C = 1)}{P(X = x\vert C = 0)}\right )}
$$
...
know the distribution of each class $P(X = x\vert C = c)$. Therefore, we will have to
...
3. $P(X = x\vert C = 1)$ = number of times $x$ occurred in the sample/$n$


## Problem when estimating from continuous distributions

We saw that the probability estimate $P(X = x\vert C = 1)$ was computed as
...
x\vert C = 1)$ is $0$ for most values of $x$.
...
When we obtain a new tweet $x$, we can compute $P(X = x\vert C =
\text{Positive})$ by computing the proportion of times that tweet
occurs in the positive training set. 
$P(X = x\vert C = \text{Negative})$ is the proportion of times that tweet
occurs in the positive training set. 
...
$$
	L = \log{\left (\frac{P(C = 1)}{P(C = 0)}\right )} + 
	\log{\left (\frac{P(X_1 = x_1, X_2 = x_2, \ldots, X_m = x_m\vert C = 1)}{P(X_1 = x_1, X_2 = x_2, \ldots, X_m = x_m\vert C = 0)}\right )}
$$
We can only compute $P(X_1 = x_1, X_2 = x_2, \ldots, X_m = x_m\vert C =
...
\begin{aligned}
	L &= \log{\left (\frac{P(C = 1)}{P(C = 0)}\right )} + 
	\log{\left ({\prod_{x_i \in \text{tweet}} \frac{P(X_i = x_i\vert C = 1)}{P(X_i = x_i\vert C = 0)}}\right )} \\
	&= \log{\left (\frac{P(C = 1)}{P(C = 0)}\right )} + 
	{\sum_{x_i \in \text{tweet}} \log{\left (\frac{P(X_i = x_i\vert C = 1)}{P(X_i = x_i\vert C = 0)}\right )}} \\
\end{aligned}
$$
