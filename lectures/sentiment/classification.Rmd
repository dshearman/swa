...
- a large sample size of bloth classes (the more the better)
...
- choose an appropriate form for each tweet (we represent the tweet as
  a document vector containig weighted term frequencies).
...
How do we compute these probabilties from our training set of labelled tweets?
...
Rather than examing all of the training tweets when classifying,
...
- If the score is zero, then we are unceratin of the sentiment.
