...
## Twiiter counts as a time series
...
* An anuual cycle in quarterly data --- often used for sales data. ($m=4$)
...
This data is counts on 45 consequetive days in the middle of 2013. 
...
Y = ts(sqrt(df2$count), freq=7, start=min(df2$day))
plot(cbind(Y,T<-filter(Y, rep(1/7,7))), plot.type="single", col=c("blue", "green"), lwd=2, ylab=expression(sqrt(count)), xlab="day")
...
If $m$ is even, wec use the adjusted version, so that the moving average 
...
mmat = function(x, m, d=0) {
  ...
  print(xtable((M=matrix(tmp, nrow=7)), digits=d), floating=FALSE, include.colnames=FALSE, comment=FALSE)
  invisible(M)
}
mmat(df2$count,7)
```
...
$S_t = \frac{1}{m} \sum_{j=1}^m \tilde{S}_j$
```{r echo=FALSE, results="asis"}
St = St - mean(St)
print(xtable(matrix(St,nrow=1), digits=3),floating=FALSE, include.rownames=FALSE, comment=FALSE)
```
