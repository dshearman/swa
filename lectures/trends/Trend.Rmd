...
Out of five hundred tweets, 17 mention topic A in a past period, and 27 in the current what is the $\chi^2$ statistic

$$\chi^2 = \frac{(Y-X)^2}{X} + \frac{(X-Y)^2}{(N-X)} = \frac{(27-17)^2}{17} + \frac{(17-27)^2}{(500-17)}$$
$$ = `r (27-17)^2/17` + `r (17-27)^2/(500-17)` = `r (27-17)^2/17+(17-27)^2/(500-17)`$$
...
Simple linear regression of $\sqrt{count}$ against day. ($n= `r nrow(df2)`$)

Given that 

\begin{align*}
\bar{x} = `r Xbar` && \bar{y} = `r Ybar`\\
SS_X = `r SSX` && SS_{XY} = `r SSXY` \\
RSS = `r RSS`
\end{align*}

What are $\hat{\beta}$ and $\hat{\alpha}$?
