...
$$Z_t = \sum_{i= -j}^j Y_{t+i} \qquad \forall \quad j < t \le n-j \qquad \text{undefined for other } t$$
...
$$Z_t = \frac{1}{2} Y_{t-j} + \sum_{i= -j+1}^{j-1} Y_{t+i} + \frac{1}{2} Y_{t+j}
...
$$ Z_t = \sum_{i= -j}^j Y_{t+i} = \sum_{i= -j}^j T_{t+i} + \sum_{i= -j}^j S_{t+i} + \sum_{i= -j}^j \varepsilon_{t+i}$$
