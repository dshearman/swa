...
Tweets can also be search for using a spatial location.
...
In fact, it is better to use a _weighted local average_
...
A weighted average replaces the $I(\ldots) = 1$ with a weight other than $1$.

## Weighted local average

$$\lambda_h(x,y) = \frac{1}{nh} \sum_i K(d_i(x,y)/h)$$

and $K$ is a _Gaussian Kernel_ and $h$ is a bandwidth.

$$ K(s) = \frac{1}{2\pi} e^{-s^2/2}$$

```{r echo=FALSE,dev="pdf", fig.width=2, fig.height=2, fig.cap=""}
par(mar=c(4,0,0,0)+0.1)
curve(dnorm(x), from=-3, to=3, axes=FALSE, xlab="distance")
box()
```
...
Then we compute the density, specifying the bandwidth with `sigma`
```{r eval=FALSE}
