...
In the previous example, we used the term frequency as the term weight
($w_{d,t} = f_{d,t}$).
By doing so, if a word appears twice in a document, a query using that word
will show that document being twice as relevant to a document
that contains the word only once.

If a document contains a word 200 times, is the word twice as relevant
to a document that contains the word 100 times? We would expect
not. As the frequency of a word increases, the relevance of the word
should not increase linearly, meaning that the weight should be a
sub-linear function of the frequency.

In this section we will examine document weights that provide a more
...
$$
	w_{d,t} = -\log{\left ( P(w\vert d) \right )}
$$
...
collection, so its probability $P(w\vert C)$ is $p = 100/1000 = 0.1$.  In a
