...
```{r, echo=FALSE}
Y = matrix(c(1,1,2,4,4,1,2,1,4,5),5,2)
print(Y)
```
\pause
...
```{r, echo=FALSE}
centres = matrix(c(1,3,4,1,2,3),3,2)
print(centres)
```
...
## Spherical K-means numerical Example

Five two dimensional data points. Let's find two clusters using Spherical K-means.
...
Compute SST, SSW and SSB for the following five data points:
...
```{r, echo=FALSE}
centers = matrix(c(1.33, 4.00, 1.33, 4.50),2,2)
print(centers)
```
